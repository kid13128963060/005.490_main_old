# V1.9.0 Git远程覆盖本地GUI工具｜merge冲突取远程｜保留本地时间线｜网络检测｜3秒倒计时n取消关闭｜批量双仓库｜任务锁防重复执行
# 变更记录：V1.8.0(冲突取本地) → V1.9.0 修改merge策略 -X theirs，冲突优先采用远程版本
...
def git_remote_override_local(repo_cwd: str) -> None:
    """
    远程仓库变更合并到本地，冲突优先取远程版本(-X theirs)，完整保留本地提交时间线
    ...
    print("==== 远程覆盖本地开始【自动合并｜冲突优先取远程｜保留本地时间线】 ====")
    ...
    # V1.9.0 修改：merge策略冲突取远程 → 使用 -X theirs
...
def build_window():
    root = tk.Tk()
    root.title("Git远程覆盖本地工具 Tkinter版 V1.9.0")
